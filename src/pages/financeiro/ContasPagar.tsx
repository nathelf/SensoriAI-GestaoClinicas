...
import { ArrowDownRight, Plus, Pencil, Trash2, Check } from "lucide-react";
...
const empty = { description: "", amount: "0", due_date: "", status: "pendente", notes: "", supplier_id: "", category_id: "" };

export default function ContasPagar() {
  const { data, loading, create, update, remove } = useCrud<AccountPayable>({ table: "accounts_payable", orderBy: "due_date", ascending: true });
  ...
  const expenseCategories = categories.filter(c => c.category_type === "despesa");
  const openCreate = () => { setEditing(null); setForm(empty); setModalOpen(true); };
  const openEdit = (p: AccountPayable) => { setEditing(p); setForm({ description: p.description, amount: String(p.amount), due_date: p.due_date, status: p.status, notes: p.notes || "", supplier_id: p.supplier_id || "", category_id: p.category_id || "" }); setModalOpen(true); };

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setSaving(true);
    const payload = { description: form.description, amount: parseFloat(form.amount), due_date: form.due_date, status: form.status, notes: form.notes || null, supplier_id: form.supplier_id || null, category_id: form.category_id || null };
  ...
  const getStatus = (item: AccountPayable) => {
    ...
    <div className="p-4 lg:p-6 max-w-5xl mx-auto">
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2"><ArrowDownRight className="w-5 h-5 text-primary" /><h1 className="text-xl font-bold text-foreground">Contas a Pagar</h1></div>
        ...
        data.length === 0 ? <div className="text-center py-12 text-muted-foreground text-sm">Nenhuma conta a pagar.</div> :
        <div className="stat-card overflow-hidden !p-0">
          <table className="w-full text-sm">
            <thead><tr className="border-b border-border/30">
              <th className="text-left py-3 px-4 text-muted-foreground font-medium">Descrição</th>
              <th className="text-left py-3 px-4 text-muted-foreground font-medium hidden sm:table-cell">Fornecedor</th>
              ...
              {data.map(item => {
                ...
                return (
                  <tr key={item.id} className="border-b border-border/20 last:border-0 hover:bg-muted/30 transition-colors">
                    ...
                    <td className="py-3 px-4 text-muted-foreground hidden sm:table-cell">{supplierName(item.supplier_id)}</td>
                    <td className="py-3 px-4 text-destructive-foreground font-semibold">R$ {Number(item.amount).toFixed(2)}</td>
      ...
      <CrudModal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? "Editar Conta" : "Nova Conta a Pagar"} onSubmit={handleSubmit} loading={saving}>
        ...
        <div className="grid grid-cols-2 gap-3">
          <FormField label="Fornecedor"><FormSelect value={form.supplier_id} onChange={e => set("supplier_id", e.target.value)}><option value="">Nenhum</option>{suppliers.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}</FormSelect></FormField>
          <FormField label="Categoria"><FormSelect value={form.category_id} onChange={e => set("category_id", e.target.value)}><option value="">Nenhuma</option>{expenseCategories.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</FormSelect></FormField>
        </div>
